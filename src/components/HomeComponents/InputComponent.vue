<template>
    <div class='text-[4vw] flex flex-col'>
        <div v-if="homeView" class='flex flex-row text-button-text-color'>
            <button @click="(event) => handleClick('HOUSE')" :class="[buttonClass, 'mr-3']">HOUSE</button>
            <button @click="(event) => handleClick('PLAYER')" :class="[buttonClass, 'ml-3']">PLAYER</button>
        </div>
        <div v-if="searchHouseFlag" class='flex flex-col'>
            <div class="flex flex-row justify-between">
                <label> House Id </label>
                <div class="flex flex-row">
                    <button @click="(event) => handleClick('BACK')"
                        :class="[backButtonClass, 'justify-start mr-2']">BACK</button>
                    <button @click="handleSearchHouse" :class="[backButtonClass, 'justify-start ml-2']">Search</button>
                </div>
            </div>
            <input @keyup.enter="handleSearchHouse" class='text-[0.75em] text-[#FCF0CC]
          font-ibm-plex-mono
          rounded-xl bg-[#545454] border-2
          outline-none p-2 border-white' v-model="houseId" />
        </div>
        <div v-if="searchPlayerFlag" class='flex flex-col'>
            <div class="flex flex-row justify-between">
                ...
                <div class="flex flex-row">
                    <button @click="(event) => handleClick('BACK')"
                        :class="[backButtonClass, 'justify-start mr-2']">BACK</button>
                    <button @click="handeSearchPlayer" :class="[backButtonClass, 'justify-start ml-2']">Search</button>
                </div>
            </div>
            <input @keyup.enter="handleSearchPlayer" class='text-[0.75em] text-[#FCF0CC]
                  font-ibm-plex-mono
                  rounded-xl bg-[#545454] border-2
                  outline-none p-2 border-white' v-model="playerID" />
